<template>
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center relative">
        ...
        <ul
            ...
            class="absolute z-10 mt-15 w-full rounded-lg border bg-white shadow dark:bg-gray-800 dark:border-gray-700"
            >
            <li
        ...
        <div class="mt-2 flex flex-wrap gap-2">
            <span
                ...
                :key="category.id"
                class="inline-flex items-center rounded-full bg-blue-100 px-3 py-1 text-sm text-blue-700 dark:bg-blue-800 dark:text-blue-200"
            >
                {{ category.name }}
                <button
...
const { categories: suggestions } = storeToRefs(categoryStore);
...
watch(selectedCategories, (value) => {
    taskStore.fetchTasks(
        value.length > 0 ? value.map(c => c.id) : null
    );
});
